...
# given
  ...
  Given /^I have a topic in forum "(.*)" with title "(.*)" and body "(.*)" and created by user "(.*)"$/ do |forum_name, title, body, username|
    find_forum(forum_name).add_topic(title, body, find_user(username))
  end

# then

  Then /^a topic in forum "(.*)" with title "(.*)" and created by user "(.*)" should be created$/ do |forum_name, title, username|
  ...
  Then /^a post in topic "(.*)" with body "(.*)" and created by user "(.*)" should be created$/ do |topic_title, body, username|

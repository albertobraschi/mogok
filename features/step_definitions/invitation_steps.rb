...
# then
  ...
  Then /^an invitation record with email "(.*)" and created by "(.*)" should be created$/ do |email, username|
    Invitation.find_by_email_and_user_id(email, find_user(username)).should_not be_nil
  end

  Then /^I should see the code for invitation with email "(.*)" and created by "(.*)"$/ do |email, username|
